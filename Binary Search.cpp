...
int main()
{
    ...
    sort(arr, arr+n);
    pfcs("Number of quires: ");
    sci(q);
    for(i=0; i<q; i++) {
        sci(x);
        j = bsearch(n, x);
        ///j = bsearchup(n, x); ///for the highest index in which x is found
        /// j = bsearchdown(n, x); /// for the lowest index in which x is found
        if(~j) printf("%d found at position %d.", x, j);
